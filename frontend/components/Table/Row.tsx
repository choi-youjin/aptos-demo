...
import type { TableField, TableRowData, TableStyle } from '@/components/Table/types';
import RowCell from '@/components/Table/RowCell';
import useMouseEffect from '@/components/Table/hooks/useMouseEffect';
...
const Row = <T extends TableRowData>({
  ...
  const colorClassName = useMemo<string>(
    () => `${TABLE_ROW_BORDER_DICT[type]} ${tailwindGroupName} ${data.className ?? ''}`,
    [type, tailwindGroupName, data.className],
  );

  const getBgClassName = useMemo(() => GET_TABLE_ROW_BG_DICT[type], [type]);
  ...
  return (
    <div
      ...
      className={`relative ${getBgClassName(index)} ${colorClassName} ${overlayClassName}`}>
      <div
